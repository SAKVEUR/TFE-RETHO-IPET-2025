...
class Personnage():
    ...
    def lancerDesCombat(self) -> int:
        ...
        if resultat_des > 0:  # Si le dé tombe sur une face d'attaque
            degats = self.persoArme.pointsDegatsArme
            print(f"{self.nomPerso} lance les dés de combat : {resultat_des}")
            print(f"Dégâts potentiels : {degats}")
        else:  # Si le dé ne tombe pas sur une face d'attaque
            degats = 0
            print(f"{self.nomPerso} lance les dés de combat : échec")
            print("Pas de dégâts infligés")
    ...
    def lancerDesDefense(self) -> int:
        ...
        if resultat_des > 0:  # Si le dé tombe sur une face de défense
            protection = self.persoArmure.pointsProtectionArmure
            print(f"{self.nomPerso} lance les dés de défense : {resultat_des}")
            print(f"Protection : {protection}")
        else:  # Si le dé ne tombe pas sur une face de défense
            protection = 0
            print(f"{self.nomPerso} lance les dés de défense : échec")
            print("Pas de protection")
    ...
    def trouverEnnemisAPortee(self, plateau, distance_max: int = 1) -> list:
        ...
        # Parcourir toutes les cases autour du personnage
        for directionX in range(-distance_max, distance_max + 1):
            for directionY in range(-distance_max, distance_max + 1):
                # Ne pas traiter la case actuelle
                if not (directionX == 0 and directionY == 0):
                    nouveauX = self.positionX + directionX
                    nouveauY = self.positionY + directionY
    ...
    Permet au personnage d'ouvrir une porte adjacente et la fait disparaître du plateau
    """
    def ouvrirPorte(self, plateau) -> bool:
        # Vérifier les cases adjacentes pour trouver une porte
        directions = [
            (0, -1),  # haut
            (0, 1),   # bas
            (-1, 0),  # gauche
        ...
        for directionX, directionY in directions:
            nouvelleX = self.positionX + directionX
            nouvelleY = self.positionY + directionY
            ...
            if 0 <= nouvelleX < plateau.nombreDeColonne and 0 <= nouvelleY < plateau.nombreDeLigne:
                # Vérifier si l'objet à cette position est une porte
                if isinstance(plateau.plateau[nouvelleY][nouvelleX], Porte):
                    porte = plateau.plateau[nouvelleY][nouvelleX]
                    if porte.estOuverte:
                        print("Cette porte est déjà ouverte.")
                        return False
                    else:
                        # Ouvrir la porte
                        porte.ouvrir()
                        # Remplacer la porte par un espace vide sur le plateau
                        plateau.plateau[nouvelleY][nouvelleX] = " "
                        print(f"{self.nomPerso} a ouvert une porte.")
                        return True
    ...
    Permet au personnage d'ouvrir un coffre adjacent et la fait disparaître du plateau
    """
    def ouvrirCoffre(self, plateau) -> bool:
        # Vérifier les cases adjacentes pour trouver un coffre
        directions = [
            (0, -1),  # haut
            (0, 1),   # bas
            (-1, 0),  # gauche
        ...
        for directionX, directionY in directions:
            ...
            # Vérifier si la position est dans les limites du plateau
            if 0 <= nouvelleX < plateau.nombreDeColonne and 0 <= nouvelleY < plateau.nombreDeLigne:
                # Vérifier si l'objet à cette position est un coffre
                if isinstance(plateau.plateau[nouvelleY][nouvelleX], Coffre):
                    coffre = plateau.plateau[nouvelleY][nouvelleX]
                    if coffre.estOuvert:
                        print("Ce coffre est déjà ouvert.")
                        return False
                    else:
                        # Ouvrir le coffre
                        coffre.ouvrir()
